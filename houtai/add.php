...
while($node=mysql_fetch_object($q)){
    $list[]=$node;
}

//简单缩进方式
$stack=array();
?><h2>价寿尚季圭、三仕应逢鼎、启志良心现、广发永长绵、宗强生贵子、圣王佑福卿、祖坐丰华地、万代庆兴隆。 四个代辈序，永远卜肆，源远流长</h2><ul class="view_node">
	<h2>节点管理	<li><a href="/index.php">点击此处进入族谱树查询上下辈</a></li></h2>
...
foreach($list as $node):
    ...
    ?><li nodeid="<?=$node->id?>"><p><span title="点击编辑节点" class="tree_edit"><?=$node->name?></span>——编号:<font color="red"><span title="ID"><?=$node->id?></span></font> 父亲编号:<span title="PID"><font color="red"><?=$node->pid?></span></font> <font color="#ffffff"><span title="左值"><?=$node->L?></span> <span title="右值"><?=$node->R?></span></font></p><a href="javascript:void('添加子节点');" title="添加子节点" class="tree_add">添加子女</a>-<a href="/admin/infoedit.php?id=<?=$node->id?>" title="详情编辑" class="info_add">详情编辑</a><?php
    if($node->R-$node->L-1){
        ?><ul><?php
    }
endforeach;
