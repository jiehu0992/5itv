...
<?php
    ...
    //父亲
    $sql = mysql_query("SELECT pid,name FROM tree_lr WHERE id=".$sql_arr['pid'],$link);
    $fa_name = mysql_fetch_assoc($sql);
    $sql_arr['dad']=$fa_name['name'];
     //母亲
    $sql = mysql_query("SELECT pid,wname FROM tree_lr WHERE id=".$sql_arr['pid'],$link);
    $ma_name = mysql_fetch_assoc($sql);
    $sql_arr['mother']=$ma_name['wname'];
    //祖父
    $sql = mysql_query("SELECT pid,name FROM tree_lr WHERE id=".$fa_name['pid'],$link);
    $gr_name = mysql_fetch_assoc($sql);
    $sql_arr['gdad']=$gr_name['name'];
    //兄弟
    $sql = mysql_query("SELECT * FROM tree_lr WHERE sex='男' and pid=".$sql_arr['pid'],$link);
	$br_name = "";
    ...
    if(empty($br_name)){$br_name ='无';}else{
     ...
     if($br_name  == "")$br_name ='无';
      ...
      //姐妹
	$sql = mysql_query("SELECT * FROM tree_lr WHERE sex='女' and pid=".$sql_arr['pid'],$link);
	$sis_name= "";
    while ( $row=mysql_fetch_array($sql)) {
    ...
    //子
    $sql = mysql_query("SELECT * FROM tree_lr WHERE sex='男' and pid=".$sql_arr['id'],$link);
	$son_name= "";
 		while ( $row=mysql_fetch_array($sql)) {
    ...
    if(empty($son_name)){$son_name='无';}
     ...
     //女
    $sql = mysql_query("SELECT * FROM tree_lr WHERE sex='女' and pid=".$sql_arr['id'],$link);
	$da_name= "";
  	while ( $row=mysql_fetch_array($sql)) {
...
if(trim($sql_arr['rank']) == ""){//如果本人排行已经定义，则直接取出，不用默认
  ...
  $sqlx = mysql_query($sql_tmp,$link);//需转化维数组


  $rank1=array("长子","次子","三子","四子","五子","六子","七子","八子","九子","十子","十一子","十二子");
  $rank2=array("长女","次女","三女","四女","五女","六女","七女","八女","九女","十女","十一女","十二女");
  $paih = "";

while ( $rowx=mysql_fetch_array($sqlx)) {//取出并拼接本人兄弟或姐妹已占用的排行
    if($paih == ""){
        $paih = $rowx['rank'];
    }else{
    ...
    if(!empty($paih)){//取出已占用的排行不为空时执行，否之直接长子或长女
        ...
        if($sql_arr['sex']=="男"){$paih_x="长子";}else{$paih_x="长女";}
    }
...
<form action="action-editinfo2.php" method="post">
 ...
 <table align="center" cellpadding="0" cellspacing="0" style="width: 80%;height:100%">
	...
	<tr>
		...
		<tr>
			...
			<td width="13%">
				...
				<option value="16">16广</option>
				<option value="17">17发</option>
				<option value="18">18永</option>
				<option value="19">19长</option>
				<option value="20">20绵</option>
				<option value="21">21宗</option>
				<option value="22">22强</option>
				<option value="23">23生</option>
				<option value="24">24贵</option>
				<option value="25">25子</option>
				<option value="26">26圣</option>
			...
			<td width="13%">
				...
				<option value="发">17发</option>
				<option value="永">18永</option>
				<option value="长">19长</option>
				<option value="绵">20绵</option>
				<option value="宗">21宗</option>
				<option value="强">22强</option>
				<option value="生">23生</option>
				<option value="贵">24贵</option>
				<option value="子">25子</option>
				<option value="圣">26圣</option>
		...
		<td width="4%" >生平：</td>
			<td colspan="4"><input type="text" id="info" name="info"  value="<?php echo $sql_arr['name']?>配<?php echo $sql_arr['wname']?>生子<?php echo $son_name; ?>，女：<?php echo $da_name; ?>。" readonly="readonly" />	</td><br>
		</tr>
	</tbody>
	...
	<tr>
			...
			<tr>
		<td width="4%" >其他：</td>
		<td colspan="5"><textarea class="form-control" id="other" name="other" rows="3" style="min-width: 70%"  value="<?php echo $sql_arr['other']?>"><?php echo $sql_arr['other']?></textarea><br>其他示例：三才未明嘉靖二、三十年代，万历十年1582年圣午科，解元。又维基百科刘三才字汝立1559年十二月初八日生，殁于1618年	</td><br>
			</tr>
		<tr>
			<td width="4%" >国籍：</td>
