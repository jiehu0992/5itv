...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (isset($_POST['isdrop']) && $_POST['isdrop']) { // 删除节点
        if ($node) {
            $num = $node->R - $node->L + 1;
            ...
            mysqli_query($link, 'UPDATE `tree_lr` SET R=R-' . $num . ' WHERE R>' . $node->R) or die('删除节点时更新R值错误！' . mysqli_error());
             exit('删除节点成功！<script>location.reload();</script>');
        } else {
            die('<font color="red">删除的节点不存在！</font>');
        }
    } elseif (isset($_POST['isswap']) && $_POST['isswap']) { // 交换两节点位置
        ...
        if ($node && $node2) {
            ...
            mysqli_query($link, 'UPDATE `tree_lr` SET L=L+IF(L>' . $node->R . ',-' . $num . ',' . $num2 . '), R=R+IF(R>' . $node->R . ',-' . $num . ',' . $num2 . ') WHERE (L>=' . $node->L . ' AND R<=' . $node->R . ') OR (L>=' . $node2->L . ' AND R<=' . $node2->R . ')') or die('交换节点时更新L值错误！' . mysqli_error());
             exit('交换节点修改成功！<script>location.reload();</script>');
        } else {
            die('<font color="red">交换的节点至少其中之一不存在！</font>');
        }
   // 移动节点到...
    } elseif (isset($_POST['ismove']) && $_POST['ismove']) {
        if ($node && $pnode) { // 从后移到前...
            $maxR = mysqli_fetch_assoc(mysqli_query($link, 'SELECT MAX(R) as maxR FROM tree_lr'))['maxR'];
            $num = $node->R - $node->L + 1;
        ...
        if ($node) {
            mysqli_query($link, 'UPDATE `tree_lr` SET `name`=\'' . addslashes($_POST['name']) . '\' WHERE id=' . $id) or die('修改节点是更新name值错误！' . mysqli_error());
            exit('修改成功！<script>location.reload();</script>');
        } else {
        ...
        if ($pid) {
            ...
            if ($pnode) {
               ...
               die('添加成功！<script>location.reload();</script>');
            } else {
...
function tree_add(ipt, pid) {
    ...
    // 发送异步 POST 请求
    $.post({ pid: pid, name: val }, function (response) {
        // 判断响应是否为 'true'
        if (response === 'true') {
            // 如果是 'true'，显示成功消息并重新加载页面
            alert('成功添加一名成员！');
            location.reload(true);
        } else {
            // 如果不是 'true'，显示添加失败的消息
...
$stack=array();
?>
	<h2>价寿尚季圭、三仕应逢鼎、启志良心现、广发永长绵、宗强生贵子、圣王佑福卿、祖坐丰华地、万代庆兴隆。 四个代辈序，永远卜肆，源远流长</h2>
	<!--
	<ul class="view_node">
	<?php
